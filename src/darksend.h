...
{
public:
    /// Is the inputs associated with this public key? (and there is 10000 MOJO - checking if valid masternode)
...
public:
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)){
            LogPrint("mojomix", "CDarksendPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Masternode. \n");
            return;
        }
